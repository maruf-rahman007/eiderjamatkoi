...
const EID_YEAR = parseInt(process.env.NEXT_PUBLIC_EID_YEAR || '2026');
...
export async function GET(req: NextRequest) {
  ...
  const mosques = await prisma.mosque.findMany({
    ...
    include: {
      prayerTimes: { where: { year: EID_YEAR }, orderBy: { voteCount: 'desc' } },
  ...
  const results: MosqueWithTimes[] = mosques
    .map((mosque) => {
      ...
      const selectedTime = mosque.prayerTimes.find((pt) => pt.isSelected) || mosque.prayerTimes[0] || null;
      const totalVotes = mosque.prayerTimes.reduce((sum, pt) => sum + pt.voteCount, 0);
...
export async function POST(req: NextRequest) {
  ...
  if (!dbUser) return NextResponse.json({ error: 'User not found — please sync first' }, { status: 404 });

  const canSubmit = await checkDailyLimit(dbUser.id);
  if (!canSubmit) return NextResponse.json({ error: 'Daily submission limit reached (5/day)' }, { status: 429 });

  const body = await req.json();
  const { name, nameBn, lat, lng, photoUrl, jamaatTimes, userLat, userLng } = body;

  if (!name || !lat || !lng || !jamaatTimes?.length)
    return NextResponse.json({ error: 'name, lat, lng, and jamaatTimes are required' }, { status: 400 });

  if (jamaatTimes.length > 4) return NextResponse.json({ error: 'Maximum 4 prayer times allowed' }, { status: 400 });
  ...
  const mosque = await prisma.$transaction(async (tx) => {
    const newMosque = await tx.mosque.create({
      data: { name, nameBn, lat, lng, photoUrl, submittedBy: dbUser.id },
    });
